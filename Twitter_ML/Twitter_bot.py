...
# Look for new mentions that contains with '@TrioRegress "', that means that some user registered a new company to predict
new_companies = []
for mention in my_new_mentions:
    if ('@' + username + '"') in mention.text:
        ...
        if (str(new_company[1:-1]).upper() + '.SA') in companies:
            tweet_new_company = 'Já tweetamos previsões de {} :D'.format(str(new_company[1:-1]).upper())
...
if (str(date.today()) in my_tweets[0].text):
    ...
    for company in companies:
        ...
        # If the Yahoo Finance still didn't updated the data
        if df.iloc[-1][1] == 0:
